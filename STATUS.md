...
### 2026-08-18 — Scheduled-Routine Security/Logic-Audit (read-only, kein Deploy)

- **Auftrag:** Automatisierte Scheduled-Task-Session ("Ressourcen-Maximierung"-Routine) — proaktiver Security/Logic-Audit ohne konkretes Briefing. Read-only Scan von `index.html` (82k Zeilen), `sw.js`, allen 25 `supabase/functions/*`, relevante Migrations. Kein Code editiert, keine Deploys.
- **Findings:** 0 CRITICAL, **1 HIGH**, 3 MEDIUM, 4 LOW. 0 hardcoded Secrets, 0 verbleibende user-facing `alert()`, 0 unsanktionierte `gs_sb_token`-Zugriffe.
- **HIGH — `mushroom-identify` + `pest-identify` ohne In-Code-Auth/Quota:** Beide Edge-Fns (Anthropic Vision, kostenpflichtig) haben **keinerlei** `req.headers`/JWT-Handling im Code (anders als `ai-proxy`/`garden-scan-analyze`/`plant-doctor-diagnose`, die alle `auth.getUser()` nutzen) und **keinen** `fn_log_ai_usage`-Call — obwohl der 2026-05-24(b)-Eintrag oben behauptet, v26.50 habe genau das für `mushroom-identify`/`pest-identify` ergänzt. Repo-Code und (mutmasslich) deployter Code sind hier entweder auseinandergelaufen, oder die damalige Changelog-Aussage war ungenau — **braucht Verifikation gegen den live-deployten Stand (Supabase Dashboard) bevor irgendwer daran patcht**. Sicherheit haengt komplett von einer nicht-im-Repo-sichtbaren Gateway-`verify_jwt`-Einstellung ab — exakt das Fehlerbild, das `i18n-translate` laut eigenem Header-Kommentar in v1-v3 schon mal hatte (kein Auth, kein Secret).
- **MEDIUM:** (1) CLAUDE.md §3.6 referenziert `gsSafeHTML` als sanktionierten Escape-Helper — **existiert nirgends im Code** (`grep "gsSafeHTML\s*="` → 0 Treffer). Reale Escaping-Arbeit läuft über `escHtml()`/`gsHTMLEscape()` (beide vorhanden, funktionieren). Doku/Code-Drift, kein aktiver XSS-Bug. (2) `index.html:31227` rendert `p.type||p.category||'fund'` unescaped in ein `title`-Attribut im Social-Feed — Sicherheit hängt an einer DB-CHECK-Constraint, die in den geprüften Migrations nicht gefunden wurde. (3) `entitlements` + `push-test` Edge-Fns haben noch das alte `ALLOWED_ORIGINS` ohne kanonisches `https://green-scan.ch` (das `ai-proxy` seit v30.37 hat) — aktuell wirkungslos, da beide Fns laut Grep keine Call-Site im Frontend haben.
- **LOW (4):** inkonsistentes Attribut-Escaping Marketplace-Card vs. Social-Feed-Foto (`index.html:20885` vs. `31233`); unescaped Pflanzen-Nickname in Options-Liste (`index.html:30959`, self-XSS-only); toter Code `gsEnrichSpeciesViaAI()` (`index.html:26162`, 0 Call-Sites, bypassed `callAI` + nutzt non-kanonische localStorage-Keys) — Löschkandidat; `_gsKeyHealthWalker()` (`index.html:60854`) macht Direct-Fetch gegen Anthropic für Key-Health-Pings, verletzt den Wortlaut von §3.4 (nur `gsTestApiKey()` sanktioniert), funktional aber harmlos (max_tokens:4, kein Content-Call).
- **Bewusst NICHT autonom gepatcht:** Auth/Quota-Fix für mushroom-identify/pest-identify fasst Live-Security- und Live-Kosten-relevanten Edge-Fn-Code an, plus die Repo-vs-Deployed-Diskrepanz muss zuerst geklärt werden (Supabase Dashboard) — daher Freigabe durch Fernando abwarten statt blind deployen.
- **Naechste (fuer wer den Fix macht):** (a) Supabase Dashboard pruefen ob `mushroom-identify`/`pest-identify` `verify_jwt=true` haben UND ob die deployte Version bereits `fn_log_ai_usage` enthaelt (Repo-Code tut es nicht); (b) falls nicht: Auth-Pattern von `ai-proxy` (`auth.getUser()` + Quota) uebernehmen; (c) `gsSafeHTML` entweder implementieren oder CLAUDE.md §3.6 auf `escHtml`/`gsHTMLEscape` umschreiben; (d) `entitlements`/`push-test` CORS-Allowlist um `green-scan.ch` ergaenzen falls sie je verdrahtet werden.
