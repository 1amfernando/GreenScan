...
| B2 | MEDIUM | `index.html` ~667× innerHTML | `gsSafeHTML` war bis v30.80 nur DOKUMENTIERT, nicht definiert (siehe §Wiederholungsfall). Seit v30.80 real vorhanden — Migration iterativ pro Modul. B8 zeigt, dass die nicht-migrierten Sinks real ausnutzbar waren. | P2: safeHTML-Migration (Helper ✓ seit v30.80, Code-Migration pending) |
...
| ~~B8~~ | ~~CRITICAL~~ | ~~Karte / Community / Marktplatz~~ | ~~Stored XSS: `map_user_finds.photo_url` roh in `<img src>` (Pull ohne User-Filter, per PATCH auf eigener Zeile setzbar → Zugriff auf `gs_sb_token`); dazu `social_posts.category`, Marktplatz `region`/`certification_label`/`photo_urls`~~ | **erledigt v30.80** |
| ~~B9~~ | ~~HIGH~~ | ~~`sw.js` Precache~~ | ~~`install` schreibt in SHELL_CACHE, fetch liest aus STATIC/IMAGE/RUNTIME → 20/23 Einträge (~4.5 MB) nie ausgeliefert, offline keine Arten-DB~~ | **erledigt v30.80** (`cacheLookup`) |
| ~~B10~~ | ~~MEDIUM~~ | ~~`reportErrorToServer` / `gsTrackEvent`~~ | ~~Telemetrie ohne Consent bzw. opt-OUT auf einem nie geschriebenen Key — Widerspruch zu privacy.html + §3.7~~ | **erledigt v30.80** (`gsHasAnalyticsConsent`, default-deny) |
| **B11** | **HIGH** | `index.html` `callAI`/`callVisionAI` | **OFFEN.** KI-Quota ist auf ~16 Call-Sites rein client-seitig (`gsAboCanUse` liest nur `localStorage.gs_abo_plan` + `gs_aicalls_*`). Nur 7 Sites übergeben `quotaFeature` → nur die sind über `fn_quota_peek`/`fn_quota_consume` server-autoritativ. `sendScanChat` hat GAR keine Quota und loggt auch nicht in `ai_daily_usage`. Root-Cause: `gsPullGlobalApiKey` legt den rohen `sk-ant-`-Key in jeden Browser. Kostet echtes Geld. | P0: AI-Proxy Schritt 3 (`fn_get_global_api_key` darf den Rohschlüssel nicht mehr herausgeben) — der `gs_feat_aiproxy`-Schalter ist selbst client-seitig und schliesst allein nichts |
| **B12** | **HIGH** | `supabase/functions/stripe-expert-checkout` | Repo-Kopie in v30.80 gefixt (`expert_id` an JWT gebunden), **LIVE-Deploy steht noch aus**. Bis dahin kann jeder für CHF 0.50 eine Experten-Verifikation im Namen eines beliebigen Experten erzeugen. | sofort deployen |
| **B13** | **MEDIUM** | `.well-known/assetlinks.json` | Enthält noch die Platzhalter `REPLACE_WITH_*_FINGERPRINT` → Android-TWA-Verknüpfung schlägt fehl, Chrome-Adressleiste bleibt in der installierten App sichtbar. | vor Play-Store-Submission |
| **B14** | **MEDIUM** | `_headers` `/assets/*` | `immutable, max-age=31536000`, aber `/assets/leaflet.js|.css` und `three.min.js` werden OHNE `?v=`-Query referenziert → kein Cache-Bust-Pfad bei einem Lib-Update. | Query-Versionierung nachziehen |
| **B15** | **LOW** | Repo → Cloudflare Pages | Ohne Build-Step ist das GANZE Repo unter `green-scan.ch/<pfad>` abrufbar und laut `robots.txt` crawlbar — inkl. `V28_FULL_AUDIT_FINDINGS.md`, `A_Z_AUDIT_FINDINGS_v28.95.json`, `supabase/migrations/*.sql`, Edge-Function-Quelltext. Keine Credentials committed (geprüft), aber Schema, RLS-Design und die eigene Schwachstellen-Liste liegen offen. | `Disallow` + ggf. Auslagern |
| **B16** | **LOW** | `_redirects` SPA-Fallback | `data/plants.v1.js?v=1` und `sw.js` werden RELATIV referenziert. Auf einem mehrsegmentigen Pfad (`/a/b`) lösen sie zu `/a/…` auf und der Fallback liefert `text/html` → `window.DB` undefined + SW-Registrierung scheitert. Aktuell latent (alle Einstiegs-URLs sind einsegmentig). | `<base href="/">` oder absolute Pfade |
