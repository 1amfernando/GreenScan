// v26.33 mushroom-identify Edge-Fn (Cowork-Auftrag AUFTRAG_CODE_v26.33)
...
Deno.serve(async (req) => {
  ...
  // 1) Knowledge: ALLE 20 Pilze laden (klein genug)
  ...
  // 2) Knowledge-Block fuer System-Prompt
  ...
  // 4) Vollstaendige Pilz-Daten + Safety-Info via v_mushroom_safety View
  ...
  if (parsed.matched_slug && typeof parsed.matched_slug === "string") {
    ...
    safetyRow = saf;
    // Lookalikes: sowohl als edible_slug ALS auch als lookalike_slug → bidirektional
  ...
  // 5) VAPKO-Kontrollstelle aus Region (wenn angegeben)
  ...
  // Fallback: irgendeine Kontrollstelle
  if (!vapkoStelle) {
    const { data } = await admin.from("mushroom_seasonal_patches")
      .select("vapko_kontrollstelle,vapko_phone,vapko_link")
  ...
  // Force vapko_required wenn confidence niedrig ODER edibility toedlich/giftig
