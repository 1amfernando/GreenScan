...
|---|---|---|---|
| 1 | **Garten-Pflanzungen haben keine Pflege.** `gs_plantings` kennt kein `tasks`; „Heute zu tun" gilt nur für Zimmerpflanzen. Der 🌳-Reiter ist ein Spiegel mit Deep-Link. | 3 | **offen** — Kalender Stufe 2 (`KALENDER-V1.md` §3.1): Vorgaben je Gartenart, nachrüstbar |
...
| 3 | **Drei Tagebücher ohne Verbindung.** Abhaken → `p.diary`; das Gartentagebuch las nur `gs_gartentagebuch`; das Cloud-Formular schreibt in `garden_diary`. | 3 | **teilweise behoben v32.46** — `gsTagebuchAlle()` liest die zwei lokalen Bücher zusammen; das Cloud-Buch folgt (Stufe 2) |
...
| 7 | **Der Notizzettel verdeckt den Pfeil der ersten Karte.** Er klebt bei 38 % Höhe am rechten Rand — genau dort, wo bei drei fälligen Aufgaben die erste Pflanzenkarte ihr „›" hat. Gemessen im Screenshot 412 px. | 2 | **offen** — Vorschlag: Zettel nach unten (bei ~62 %), dort ist bei gefüllter Liste Kartenfläche ohne Bedienelement; oder rechter Innenabstand der Karten, solange der Zettel sichtbar ist |
| 8 | **`buildPlantCard` ist tot** — definiert (~Z. 34680), nie aufgerufen; `gsNewPlantCard` rendert seit v26.x. 60 Zeilen mit `p.water`/`p.sun`-Punkten, die es im Modell nicht gibt. | 1 | **offen** — ersatzlos entfernen (Stufe 2, mit `wiring_check`) |
...
| 10 | **Das Tagebuch koppelt Pflanzen über den Namen**, nicht die Id — zwei „Tomate" sind dieselbe. | 2 | **abgemildert v32.46** — `gsTagebuchAlle` löst den Namen zur Id auf, wo eindeutig; `pflanze_id` beim Schreiben folgt (Stufe 2) |
...
Stufe 2 aus `KALENDER-V1.md` §6 schliesst 1, 3, 8 und 10. Punkt 7 ist eine
CSS-Zeile mit einer Messung davor (die Zettel-Position gegen die Karten-
Pfeile bei 0, 1, 3 und 8 fälligen Aufgaben). Alles davon mit Fall im
Prüfstand, und die Befunde dieser Liste gehen durch die gegnerische
Prüfung, sobald das Kontingent es zulässt.
